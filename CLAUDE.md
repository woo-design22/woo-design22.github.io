...
- `ai-council/index.html` — 「AI 회의실」: **회사가 다른 모델들을 한 방에 앉히고 순번제로 토론시킨 뒤**
  ...
  연결은 세 갈래 — **우리 회의실**(counsel-proxy 의 `/room`, 키가 브라우저에 안 내려온다) ·
  오픈라우터 직통(내 키) · 앤트로픽 직통(클로드만). 모델 344개는 목록 API 로 직접 받아 온다.
...
- **라우트**: (기본 POST) 웹 상담 · `/triage` 구급대원 · **`/room` AI 회의실** ·
`/telegram` 텔레그램 봇 · `/kakao` 카카오 챗봇.

**`/room` 만 다른 점**: 여기만 **오픈라우터**를 부른다(GPT·클로드·제미나이를 한 키로 쓰려고).
그래서 키(`OPENROUTER_KEY`)도 상한도 이 구역 안에서 따로 논다.
- **모델 허용목록을 손으로 적지 않고 값으로 자른다** — 오픈라우터 모델 표를 받아 한 시간 들고 있으면서
  입력 $6 / 출력 $30(100만 토큰당)을 넘으면 거부한다. `o1-pro`($150/$600) 같은 것이 한 번에 통장을 비운다.
  **표를 못 받으면 통과시키지 않는다** — 값을 모르는 채 부르는 것이 제일 위험하다.
- 비밀번호는 `ROOM_TOKEN_SHA` 하나로 확인한다. 클라이언트가 보낸 표를 한 번 더 뭉개 비교하고,
  비교는 **길이·내용에 시간이 안 걸리게**(`sameSecret`) 한다. **비어 있으면 라우트를 통째로 닫는다.**
- 레이트리밋이 상담(60초 8회)과 다르다 — 회의 한 판이 7~19회를 몰아 부르므로 **60초 40회**다.
- `max_tokens` 는 4,500 에서, 노력도는 `high` 에서 자른다(`xhigh`·`max` 는 `high` 로 깎는다 —
  모르는 값이라고 `low` 로 떨어뜨리면 답의 질이 뚝 떨어져 버그처럼 보인다).
- 검증은 `deno` 없이도 된다: 진짜 파일에 `Deno` 껍데기만 씌워 Node 로 돌린다
  (복사본을 만들면 그 순간 검증이 거짓말을 한다).
...
- **키를 여럿이 나눠 쓰려면 프록시밖에 없다.** 정적 페이지는 키를 아무리 숨겨도 브라우저가 결국
  평문으로 쥐어야 보내지므로 F12 네트워크 탭에 그대로 찍힌다 — 「여럿이 쓰되 아무도 못 본다」는
  서버 없이 성립하지 않는다. 그래서 기본 연결이 `room`(counsel-proxy 의 `/room`)이다.
  이때 **비밀번호가 껍데기가 아니라 진짜 자물쇠가 된다** — 비밀번호에서 뽑은 표
  (`sha256('ai-council-auth:' + pw)`, `council_rt` 에 보관)를 프록시에 같이 보내고 서버가 확인한다.
  비밀번호를 모르면 서버가 아예 일을 안 한다.
  프록시가 자르는 노력도 상한(`PROXY_MAX_EFFORT`)은 **앱에서도 미리 감춘다** —
  고를 수 있는데 거부당하면 그게 더 나쁘다.
