...
- **자료(파일)를 올릴 수 있다.** 개수는 안 막고 **모델에 보내는 글자 수**(`FILE_BUDGET` 12,000)를 막는다 —
  ...
  **`.docx` 는 라이브러리 없이 읽는다** — zip 중앙 디렉터리를 훑어 `word/document.xml` 만 꺼내
  브라우저 기본 `DecompressionStream('deflate-raw')` 으로 푼다(CDN 금지 규칙 때문).
  **`.pdf` 는 되면 읽고 안 되면 솔직히 실패를 알린다** — 뽑아낸 글자의 「글자다움 비율」이 0.7 아래면
  거부한다. 반쯤 깨진 글자를 회의에 흘려보내면 회의 전체가 헛돈다.
  hwp·xlsx·pptx 는 못 읽는다고 분명히 말해 준다. 파일은 **메모리에만** 둔다(localStorage 는 금방 찬다).
