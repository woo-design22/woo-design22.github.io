        ...
        if wide and made:
            ...
            if reg:
                ...
                if dmax > 1.0:
                    ...
                    # **테두리는 붙들어 둔다.** 자국을 없애려고 세 번 시도했고 셋 다
                    # 되돌렸다 — 테두리를 풀면 막이 안쪽으로 끌려가 속이 비치고(89->924),
                    # 바깥 세 겹을 풀면 멀쩡한 살결이 끌려와 주름이 지고, 두 겹만 풀어도
                    # 지표가 나빠졌다(밝은 0->11, 속 124->493). 남은 자국은 기하가 아니라
                    # **결의 차이**다 — 주변 살결에는 힘줄 자국이 있는데 막은 매끈하다.
                    # 원본에 없는 면이라 그 결을 지어낼 근거가 없어 그대로 둔다.
