...
(function (root, factory) {
  ...
  var TUNING = {
    ...
    kick: {
      ...
      // 8차 지시로 면적 15% 축소(48 → 44.26), **2026-08-27 지시로 거기서 다시 20% 축소**.
      // 면적이므로 **한 변에 √0.8 = 0.8944** 를 곱한다: 44.26 → 39.59 (넓이 88.5² → 79.2² = 80%).
      // 클라이언트의 이펙트도 이 값을 그대로 읽어 그리므로 그림 크기가 저절로 따라온다(§5.2).
      fartHalf: 39.59,    // 정사각형 반변. 캐릭터 kickRange 배율이 곱해진다(덩치형 62)
      // **앞차기도 정사각형이다**(2026-08-26 지시). 부채꼴은 어디까지 맞는지 눈으로 알 수 없었다.
      // 뒷모서리가 선수 중심을 지나고 앞으로 2×kickHalf 만큼 뻗으며 옆으로 ±kickHalf.
      // 옛 부채꼴(반지름 56 · ±60°)의 넓이 3,284 에 맞춘 28.7 에서,
      // **2026-08-27 지시로 면적 30% 축소** — √0.7 = 0.8367 을 곱해 28.7 → 24.01 (넓이 3,294 → 2,306).
      kickHalf: 24.01,
      // **논스톱 슛**(2026-08-26): 공을 잡지 않은 채 D 를 눌렀을 때, 발 앞 아주 좁은 범위에
      // 굴러오는 공이 있으면 그대로 강하게 차 낸다. 발차기 범위보다 훨씬 좁아야 '타이밍 기술'이 된다.
      // 이건 **사람을 때리는 네모가 아니라 공을 찾는 원**이라 앞차기 축소와 무관하게 그대로 둔다.
      volleyR: 26,        // 논스톱 판정 반지름 (발 앞 carryOffset 지점 기준)
