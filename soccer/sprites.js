...
(function (global) {
  ...
  var GW = 24, GH = 26, BASE = 24, CX = 12;
  var OUTLINE = '#191426';           // 도트 외곽선(짙은 남보라)
  ...
  function getPose(pose, f) {
    ...
    } else if (pose === 'kick' || pose === 'backkick') {
      var back = pose === 'backkick';
      s.kick = back ? -1 : 1;
  ...
  function frameCount(pose) {
    return pose === 'run' ? 4 : (pose === 'kick' || pose === 'backkick') ? 3 : 2;
  }
  function frameOf(pose, t, phase) {
    if (pose === 'run')  return Math.floor(t * 11) & 3;
    if (pose === 'stun') return Math.floor(t * 9) & 1;
    if (pose === 'kick' || pose === 'backkick') return phase < 0.3 ? 0 : phase < 0.62 ? 1 : 2;
  ...
  function draw(ctx, o) {
    ...
    if (pose !== 'run' && pose !== 'kick' && pose !== 'backkick' &&
        pose !== 'stun' && pose !== 'ult') pose = 'idle';
    var f = frameOf(pose, t, phase) % frameCount(pose);

    ctx.save();
    ctx.imageSmoothingEnabled = false;

    ellipse(ctx, x + r * 0.10, y - r * 0.05, r * 0.90, r * 0.32, 'rgba(0,0,0,0.32)');
    ...
    ctx.drawImage(img, -CX * S, -(BASE + 1) * S, GW * S, GH * S);
