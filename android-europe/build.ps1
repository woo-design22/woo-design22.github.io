# 우리가족 유럽여행 APK 빌드 (Gradle 없이 Android SDK 도구만 사용)
#
...
# 웹 앱 원본을 assets 로 옮긴다. 웹/APK 가 같은 index.html 을 쓰는 게 목적.
# 이 게임은 index.html 말고도 음악 2개와 사진 6장이 필요하고, 사진은 .bin(XOR)이라
# WebView 의 file:// 에서 fetch 로 못 읽는다. 그래서 파이썬이 .jpg 로 되돌려 넣는다.
# 런처 아이콘은 res\mipmap-* 에 따로 들어간다(WebView 는 manifest 를 쓰지 않는다).
Write-Host "== 0/5 assets 준비 ==" -ForegroundColor Cyan
& python "$ROOT\prepare_assets.py"
if ($LASTEXITCODE -ne 0) { throw "prepare_assets.py failed" }
