...
def build(only=None):
    ...
    # 도시 간(IC-)·경기 광역(GG-, D-109)도 함께 걷어낸다 — 그것들은 여기서 붙인 정류장
    # 뒤의 노드를 가리키고 있어서, 남겨 두면 「지우려는 노드를 아직 쓰는 노선이 있다」로
    # 막힌다(실제로 막혔다). 노드 번호가 다시 매겨지므로 끝나면
    # build_ggbus.py → build_intercity.py 순으로 뒤이어 돌려야 한다.
    had_ic = sum(1 for r in routes if str(r.get('id', '')).startswith('IC-'))
    had_gg = sum(1 for r in routes if str(r.get('id', '')).startswith('GG-'))
    routes[:] = [r for r in routes
                 if not str(r.get('id', '')).startswith(('CB-', 'IC-', 'GG-'))]
    if had_ic or had_gg:
        C.log('  도시 간 %d개 · 경기 광역 %d개도 함께 걷어냈다 — 끝나면 '
              'build_ggbus.py, build_intercity.py 를 다시 돌릴 것' % (had_ic, had_gg))
